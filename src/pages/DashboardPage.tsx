...
import { BookOpen, ArrowUpCircle, TrendingUp, Clock, Sparkles, Wrench, Zap, ArrowRight, Shield } from 'lucide-react';
...
export default function DashboardPage() {
    ...
    return (
        <DashboardLayout>
            <DashboardContent user={user} courses={courses} />
        </DashboardLayout>
    );
}
...
function DashboardContent({ user, courses }: { user: any, courses: any[] }) {
    const totalContent = courses.length;
    const freeContent = courses.filter(c => c.tierRequired === 'free').length;
    const tier1Content = courses.filter(c => c.tierRequired === 'tier1').length;
                        ...
                        <Link to={`/courses/${latestCourse.id}`} className="group block">
                            <div className="p-6 bg-white/[0.03] border border-white/[0.06] rounded-2xl group-hover:bg-white/[0.06] group-hover:border-white/10 transition-all duration-300 relative overflow-hidden h-full">
                                ...
                                <div className="relative">
                                    ...
                                    <p className="text-white/35 text-xs line-clamp-2 leading-relaxed mb-3">{latestCourse.description}</p>
