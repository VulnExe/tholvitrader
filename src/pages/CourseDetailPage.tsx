    ...
    useEffect(() => {
        if (id) {
            getCourse(id).then(data => {
                setCourse(data);
                if (data?.content?.length) setActiveSection(data.content[0]);
                setLoading(false);
            });
        }
    }, [id, getCourse]);

    const hasAccess = user && course ? canAccessContent(user.tier, course.tierRequired) : false;

    return (
        <DashboardLayout>
            <div className="max-w-5xl">
                <button onClick={() => navigate('/courses')} className="flex items-center gap-2 text-sm text-white/40 hover:text-white/60 mb-6 transition-colors">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <p className="text-white/20 text-sm">Course not found</p>
                    </div>
                ) : (
                    <div className="space-y-6">
                        ...
                        <div className="p-6 bg-white/5 border border-white/10 rounded-2xl">
                            <div className="flex items-center gap-3 mb-3">
                                <TierBadge tier={course.tierRequired} />
                                ...
                                <div className="lg:col-span-1 p-4 bg-white/5 border border-white/10 rounded-2xl space-y-1 max-h-[600px] overflow-y-auto">
                                    ...
                                    {course.content.map((section, i) => (
                                        <button
                                            ...
                                            className={`w-full text-left px-3 py-3 rounded-xl flex items-center gap-3 transition-all ${activeSection?.id === section.id ? 'bg-purple-500/10 text-white border border-purple-500/20' : 'text-white/50 hover:bg-white/5'}`}
                                        >
                                            <span className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center text-[10px] font-bold shrink-0">
                                                {i + 1}
                                            </span>
