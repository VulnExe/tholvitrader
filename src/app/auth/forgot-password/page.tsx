...
export default function ForgotPasswordPage() {
        ...
        return (
            <div className="text-center animate-fade-in">
                ...
                <p className="text-white/40 text-sm mt-3 max-w-sm mx-auto">
                    We&apos;ve sent a password reset link to <span className="text-white/60">{email}</span>. Please check your inbox and spam folder.
                </p>
                <Link
                    href="/auth/login"
                    className="inline-flex items-center gap-2 mt-8 px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-white/70 text-sm font-medium hover:bg-white/10 transition-all"
                >
    ...
    return (
        <div>
            ...
            <Link href="/auth/login" className="inline-flex items-center gap-2 text-sm text-white/30 hover:text-white/60 transition-colors mb-6">
                <ArrowLeft className="w-4 h-4" />
                Back to login
            </Link>
