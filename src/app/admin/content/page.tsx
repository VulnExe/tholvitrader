...
import { Plus, Edit2, Trash2, BookOpen, FileText, Eye, EyeOff, Wrench } from 'lucide-react';
...
export default function AdminContentPage() {
    ...
    const handleSave = async (e: FormEvent) => {
        ...
        if (activeTab === 'courses') {
            if (editingId) {
                await updateCourse(editingId, { title, description, tierRequired: tier });
            } else {
                await addCourse({ title, description, tierRequired: tier, published: true });
            }
        } else if (activeTab === 'tools') {
            if (editingId) {
                await updateTool(editingId, { title, description, tierRequired: tier });
            } else {
                await addTool({ title, description, tierRequired: tier, published: true });
            }
        } else if (activeTab === 'blogs') {
            if (editingId) {
                await updateBlog(editingId, { title, content, preview, tierRequired: tier });
            } else {
                await addBlog({
                    title, content,
