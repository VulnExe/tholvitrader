...
import { User, Payment, Course, Tool, UserTier, PaymentStatus, Notification, AdminStats } from './types';
        ...
        (set, get) => ({
            ...
            _loadProfile: async (authUser: any) => {
                ...
                try {
                    ...
                    if (profile) {
                    ...
                    } else if (!existingUser || existingUser.id !== authUser.id) {
                        ...
                        // Fetch failed BUT we have an existing user (from persist)
                        // Just mark as initialized and keep the cached premium role
                        console.log('Using cached user profile after fetch failure');
            ...
            fetchCourses: async () => {
                ...
                if (data) {
                    const mappedCourses: Course[] = data.map(c => ({
                        ...
                        realPrice: c.real_price,
                        salePrice: c.sale_price,
                        originalCourseLink: c.original_link,
            ...
            getCourse: async (id) => {
                ...
                if (data) {
                    return {
                        ...
                        originalCourseLink: data.original_link,
            ...
            addCourse: async (courseData) => {
                const { data, error } = await supabase
                    ...
                    .insert({
                        ...
                        real_price: courseData.realPrice,
                        sale_price: courseData.salePrice,
                        original_link: courseData.originalCourseLink
                    })
                    .select()
                    .single();
            ...
            updateCourse: async (id, data) => {
                const { error } = await supabase.from('courses').update({
                    ...
                    original_link: data.originalCourseLink,
                    updated_at: new Date().toISOString()
                }).eq('id', id);
            ...
            fetchTools: async () => {
                ...
                if (data) {
                    const mappedTools: Tool[] = data.map(t => ({
                        ...
                        realPrice: t.real_price,
                        salePrice: t.sale_price,
                        originalCourseLink: t.original_link,
            ...
            addTool: async (toolData) => {
                const { error } = await supabase.from('tools').insert({
                    ...
                    real_price: toolData.realPrice,
                    sale_price: toolData.salePrice,
                    original_link: toolData.originalCourseLink
                });
                if (error) return { success: false, error: error.message };
